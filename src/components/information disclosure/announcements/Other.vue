  ...
  <el-container>
    <el-main v-loading="loading" class="fact-container">
      <div class="header-name title">
        <p>{{ $t('message.Other announcements') }}</p>
      </div>
...
<script setup>
  ...
  const getFacts = () => {
    ...
    axios
      .get(
        `${url}/announcement/main/?type=others&page=${currentPage.value}&report_type=2${dateValue.value.length ? `pub_date_lte=${dateValue.value[1].toISOString()}pub_date_gte=${dateValue.value[0].toISOString()}` : ''}&search=${factSearchInput.value}`,
        {
          method: 'get',
        },
      )
