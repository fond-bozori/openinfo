<template>
  <div class="d-flex gap-3 mb-3 mt-3 align-items-center">
    ...
    <el-select v-model="statusOfOrganizations" style="width: 200px">
      <el-option :label="t('With facts')" value="with_fact6"></el-option>
      <el-option :label="t('Without Facts')" value="without_fact6"></el-option>
    </el-select>
    <el-button
      :icon="Search"
      color="#124483"
  ...
  <div v-loading="loading" class="trade">
    ...
    >
      ...
      <el-table-column :label="t('message.meeting_date')" align="right">
        <template #default="scope">
          <p>
            {{
              scope.row.meeting_date
                ? moment.utc(scope.row.meeting_date).format('DD.MM.YYYY ')
                : '-'
            }}
          </p>
        </template>
      ...
      <el-table-column
        v-if="statusOfOrganizations === 'with_fact6'"
        :label="t('message.pub_date')"
        align="right"
      >
        <template #default="scope">
          <p>
            {{
              scope.row.fact6_pub_date
                ? moment.utc(scope.row.fact6_pub_date).format('DD.MM.YYYY ')
                : '-'
            }}
          </p>
        </template>
      ...
      <el-table-column
        v-if="statusOfOrganizations !== 'with_fact6'"
        :label="t('message.last_fact6')"
        align="right"
      >
        <template #default="scope">
          <p>
            {{
              scope.row.last_fact6_pub_date
                ? moment
                    .utc(scope.row.last_fact6_pub_date)
                    .format('DD.MM.YYYY ')
                : '-'
            }}
          </p>
      ...
      <el-table-column :label="t('message.due_days')" align="right">
        <template #default="scope">
          <el-tag
            effect="dark"
            style="width: 50px"
...
<script setup>
  ...
  const statusOfOrganizations = ref('with_fact6')
  ...
  const fetchData = async () => {
    loading.value = true
    try {
      const response = await http.get(`/home/organizations-without-fact6/`, {
        params: {
      ...
      tableData.value =
        statusOfOrganizations.value === 'with_fact6'
          ? response.data?.with_fact6
          : response.data?.without_fact6
    } catch (error) {
