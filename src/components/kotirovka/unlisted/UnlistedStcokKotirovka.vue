<template>
  <div v-loading="loading" class="trade">
    ...
    >
      ...
      >
        <template #default="scope">
          {{ scope.row?.high?.toLocaleString('ru') }}
        </template>
      </el-table-column>
      <el-table-column
        prop="low_price"
      ...
      >
        <template #default="scope">
          {{ scope.row?.low?.toLocaleString('ru') }}
        </template>
      </el-table-column>
      <el-table-column
        prop="change"
      ...
      >
        <template #default="scope">
          <p
            class="fw-medium"
            :style="{ color: scope.row.change_amount < 0 ? 'red' : 'green' }"
          >
            {{ financial(scope.row.change_amount) }}
          </p>
        </template>
      </el-table-column>
      <el-table-column
      ...
      >
        <template #default="scope">
          <p>{{ scope.row.total_quantity.toLocaleString('ru') }}</p>
        </template>
      </el-table-column>
      <el-table-column
        prop="volume"
      ...
      >
        <template #default="scope">
          <p class="d-flex align-items-center justify-content-center gap-1">
            {{ moment(scope.row.trade_time).format('HH:mm') }}
...
<script setup>
  ...
  const fetchTradeData = async (
    ...
    loading.value = true
    try {
      const response = await http.get(`iuzse/otc-stock-screener/?mkt_id=SMS`, {
        params: {
