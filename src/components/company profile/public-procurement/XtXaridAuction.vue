  ...
  <div class="table-responsive">
    <el-table v-loading="loading" :data="FetchedData" style="width: 100%">
      ...
      <el-table-column prop="name" :label="t('message.category_name')">
        <template #default="scope">
          <p>1. {{ scope.row.meta.good_maps[0]?.name }}</p>
          <a
            target="_blank"
            :href="`https://xt-xarid.uz/procedure/${scope.row.id}/core`"
          >
            <el-tag>{{ $t('message.more') }}</el-tag></a
          >
        </template>
      </el-table-column>
      <el-table-column
      ...
      <el-table-column
        prop="totalcost"
        :label="t('message.starting_price')"
        align="right"
      >
        <template #default="scope">
          {{ scope.row.start_price?.toLocaleString('ru') }}
          {{ scope.row.currency }}
        </template>
      </el-table-column>
      <el-table-column
        prop="totalcost"
        :label="t('message.lowest_price')"
        align="right"
      >
        <template #default="scope">
          {{ scope.row.last_price?.toLocaleString('ru') }}
          {{ scope.row.currency }}
        </template>
      </el-table-column>
    </el-table>
  ...
  const getUzexData = () => {
    ...
    http
      ...
      .then((response) => {
        ...
        console.log(FetchedData.value, 'something')
