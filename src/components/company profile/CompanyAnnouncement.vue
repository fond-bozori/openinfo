<template>
  ...
  >
    ...
    <el-table-column :label="$t('message.Download')" align="center" width="200">
      <template #default="scope">
        <a
          v-if="AnnounceTypeValue !== 'charter'"
          :href="`https://openinfo.uz/ru/announce/to_pdf/${scope.row.id}/`"
          download
          ><img
        ...
        <router-link
          v-if="AnnounceTypeValue !== 'charter'"
          :to="{ name: 'announce-by-id', params: { id: scope.row.id } }"
        >
          <img
...
<script setup>
  ...
  const selectAnnounceType = (e) => {
    ...
    updateQueryParameters(
      {
        'announce-type': AnnounceTypeValue.value,
      },
      true,
    ) // Reset the page number to 1
      .then(getFacts)
  ...
  const getFacts = () => {
    ...
    axios
      .get(
        `${url}/announcement/main/?organization_id=${id.value}&an_type=${AnnounceTypeValue.value}&page_size=5&page=${currentPage.value}${`&pub_date__lte=${endDate.value ? moment(endDate.value).format(DateFormat) : ''}&pub_date__gte=${startDate.value ? moment(startDate.value).format(DateFormat) : ''}`}`,
        {
          method: 'get',
        },
      )
  ...
  watch(activeCollapse, (newValue) => {
    if (activeCollapse.value === 'announcements') {
      getFacts()
    }
  })
  ...
  onMounted(() => {
    if (activeCollapse.value === 'announcements') {
      AnnounceTypeValue.value =
        route.query['announce-type'] ?? announceTypes[0].value
